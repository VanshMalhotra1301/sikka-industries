...
class UserSession(db.Model):
    """Tracks active and historical user sessions for security purposes."""
    ...
    # E.g., 'Active', 'Logged Out', 'Session Timeout'
    ...
    def __repr__(self):
        return f'<UserSession {self.user_id} [{self.status}]>'
    ...
    def duration_minutes(self):
        if self.logout_time:
            ...
            delta = datetime.utcnow() - self.login_time
            return round(delta.total_seconds() / 60, 2)


class ActivityLog(db.Model):
    """General activity intelligence engine log."""
    __tablename__ = 'activity_logs'

    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True) # Nullable for system events
    module = db.Column(db.String(100), nullable=False) # e.g., 'CRM', 'Sales', 'Inventory'
    action_type = db.Column(db.String(100), nullable=False) # e.g., 'Added Customer', 'Deleted Invoice'
    description = db.Column(db.Text)
    ip_address = db.Column(db.String(50))
    device_info = db.Column(db.Text)
    date = db.Column(db.DateTime, default=datetime.utcnow)
    ...
    def __repr__(self):
        return f'<ActivityLog {self.action_type} in {self.module}>'


class AuditLog(db.Model):
    """Field-level audit trail for high-fidelity data tracking."""
    __tablename__ = 'audit_logs'

    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)
    model_name = db.Column(db.String(100), nullable=False) # e.g., 'Customer', 'Product'
    ...
    def __repr__(self):
        return f'<AuditLog {self.model_name}:{self.record_id} [{self.field_name}]>'


class SystemNotification(db.Model):
    """Real-time Admin/Owner notifications."""
    __tablename__ = 'system_notifications'

    id = db.Column(db.Integer, primary_key=True)
    message = db.Column(db.Text, nullable=False)
    alert_level = db.Column(db.String(50), default='info') # 'info', 'warning', 'danger'
    module = db.Column(db.String(100), nullable=True)
    is_read = db.Column(db.Boolean, default=False)
    target_role = db.Column(db.String(50), nullable=True) # If null, all admins/owners
    date = db.Column(db.DateTime, default=datetime.utcnow)

    def __repr__(self):
        return f'<SystemNotification {self.alert_level}>'
